...
class AutoImprovementService:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.search(r'except\s*:', stripped):
                improvements.append(Improvement(
                    ...
                    suggested_code=stripped.replace('except:', 'except Exception:'),
                    priority=7,
                    confidence=0.9,
    ...
    def analyze_codebase(self, working_dir: str, max_files: int = 100, project_name: str = '') -> Dict:
        """Analyze entire codebase for improvements"""
        all_improvements = []
        file_count = 0
        
        # File extensions to analyze
        code_extensions = {'.py', '.js', '.jsx', '.ts', '.tsx', '.html', '.htm', '.css', '.scss', '.sass', '.json', '.md', '.yml', '.yaml', '.xml', '.sql', '.sh', '.bash', '.zsh', '.go', '.rs', '.java', '.cpp', '.c', '.h', '.hpp', '.cs', '.php', '.rb', '.swift', '.kt', '.scala', '.clj', '.lua', '.r', '.m', '.mm', '.dart', '.vue', '.svelte'}
        
        skip_dirs = {'.git', '.svn', '.hg', '__pycache__', 'node_modules', '.venv', 'venv', 'env', 
                     'dist', 'build', '.next', '.nuxt', 'target', 'bin', 'obj', '.idea', '.vscode', '.pytest_cache', '.mypy_cache', '.coverage', 'htmlcov', '.tox', '.eggs', '*.egg-info'}
                ...
                if ext in code_extensions:
                    try:
                        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                            ...
                            # If project_name is provided, prepend it to the path
                            if project_name:
                                # Ensure path starts with project name
                                if not rel_path.startswith(project_name + '/'):
    ...
    def apply_auto_improvements(self, improvements: List[Dict], working_dir: str, project_name: str = '') -> Dict:
        ...
        for file_path, file_improvements in by_file.items():
            ...
            # Get full file path - handle project path prefix
            clean_path = file_path.lstrip('/')
            if project_name and clean_path.startswith(project_name + '/'):
                clean_path = clean_path[len(project_name) + 1:]
            
            full_path = os.path.join(working_dir, clean_path)
