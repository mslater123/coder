...
def register():
    ...
    try:
        ...
        # Ensure user code root exists: backend/code/<user_uuid>/ (legacy / non-project paths)
        try:
            from src.api.routers.code_editor_routes import get_user_code_directory
            get_user_code_directory(user_id)
        except Exception as dir_err:
            print(f"Warning: user registered but user code directory setup failed: {dir_err}")
...
def login():
    ...
    try:
        ...
        # Accept username or email in `username` (or legacy `email`) field
        identifier = (data.get('username') or data.get('email') or '').strip()
        password = data.get('password', '')
        
        if not identifier or not password:
            return jsonify({'success': False, 'error': 'Username or email and password required'}), 400
        
        id_lower = identifier.lower()
        user = User.query.filter(
            or_(
                func.lower(User.username) == id_lower,
                and_(User.email.isnot(None), func.lower(User.email) == id_lower),
            )
        ).first()
