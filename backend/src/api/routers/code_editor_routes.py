...
from src.models import db, CodeEditorAgent, GPU, CodebaseAnalysis
...
from src.services.codebase_analysis_service import calculate_directory_hash
from src.services.auto_improvement_service import AutoImprovementService
from src.services.project_file_cache_service import invalidate_file_cache
from src.services.venv_service import (
    detect_venv,
    create_venv,
    install_package,
...
# User/project workspaces live under backend/code/
CODE_DIR = os.path.join(str(BACKEND_ROOT), "code")
...
def _require_file_ops_user(request):
    """Resolve current user for code-editor file APIs; return (user_id, None) or (None, (jsonify_response, status))."""
    from src.models import User

    user_id = get_current_user_id(request)
    if not user_id:
        return None, (
            jsonify({"success": False, "error": "User authentication required. Please log in."}),
            401,
        )
    user = User.query.get(user_id)
    if not user:
        return None, (
            jsonify({"success": False, "error": "User not found. Please log in again."}),
            404,
        )
    return user_id, None
...
def execute_with_agent(agent_id):
    ...
    try:
        ...
        if agent.agent_type in ['local', 'client']:
            ...
            from src.models import LLMQuery
            import uuid
...
def list_files():
    ...
    try:
        user_id, auth_err = _require_file_ops_user(request)
        if auth_err:
            return auth_err
        ...
        print(f"User ID received: {get_current_user_id(request)}")
...
def read_file():
    ...
    try:
        ...
        # Match write/list: do not let working_dir override project root when project_id is set
        working_dir_param = request.args.get('working_dir', '')
        if working_dir_param and not project_id:
            working_dir_param = get_working_directory(working_dir_param)
            if is_path_allowed(working_dir_param, CODE_DIR) or is_path_allowed(
                working_dir_param, get_user_code_directory(user_id)
            ):
                working_dir = working_dir_param
        ...
        if not is_path_inside_directory(full_path, working_dir):
            return jsonify({
                'success': False,
                'error': 'Invalid path'
            }), 400
...
def write_file():
    ...
    try:
        ...
        create_directory_only = data.get('is_directory') is True
        ...
        if create_directory_only or original_path.rstrip().endswith('/'):
            path = path.rstrip('/')
            if not path or '..' in path or os.path.isabs(path):
                return jsonify({'success': False, 'error': 'Invalid path'}), 400
            full_path = os.path.normpath(os.path.join(working_dir, path))
            if not is_path_inside_directory(full_path, working_dir):
                return jsonify({'success': False, 'error': 'Path outside project directory'}), 400
            try:
                os.makedirs(full_path, exist_ok=True)
            except OSError as e:
                return jsonify({'success': False, 'error': f'Failed to create folder: {str(e)}'}), 500
            return jsonify({'success': True, 'path': '/' + path.replace(os.sep, '/'), 'type': 'folder'}), 200
...
def delete_file():
    ...
    try:
        ...
        if project_id and len(project_id) > 10 and '-' in project_id:
            ...
            if project:
                ...
                if user_attached or project.created_by == user_id:
                    # Match write/list: code/{project_uuid}/
                    working_dir = get_project_directory(project_id)
                else:
                    return jsonify({
                        'success': False,
        ...
        # Do not override project working_dir with working_dir query param (matches write/list/read)
        working_dir_param = request.args.get('working_dir', '')
        if working_dir_param and not project_id:
            working_dir_param = get_working_directory(working_dir_param)
...
def analyze_codebase_endpoint():
    ...
    try:
        ...
        from src.services.codebase_analysis_service import analyze_codebase as analyze_codebase_service
