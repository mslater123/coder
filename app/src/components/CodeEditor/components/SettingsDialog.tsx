      ...
      <div className="modal-content modal-content-large settings-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="settings-container">
          ...
          <div className="settings-content">
            ...
            {settingsTab === 'agents' && (
              <div className="settings-section">
                <div className="settings-section-header agents-header">
                  <div className="agents-header-content">
                    <h3>AI Agents</h3>
                    <p className="agents-header-description">Manage your AI agents for code assistance</p>
                  </div>
                  <button 
                    className="btn-primary agents-new-btn"
                    onClick={() => {
                      setNewAgent({
                        name: '',
                        agent_type: 'local',
                  ...
                  >
                    <span className="agents-new-icon">+</span>
                    New Agent
                  </button>
                </div>
                <div className="agents-list">
                  {agents.length === 0 ? (
                    <div className="agents-empty-state">
                      ...
                      <div className="agents-empty-title">No AI Agents</div>
                      <div className="agents-empty-description">Create an AI agent to start using AI-powered code assistance</div>
                    </div>
                  ) : (
                    agents.map((agent) => (
                      <div key={agent.id} className="agent-card">
                        <div className="agent-card-header">
                          <div className="agent-card-title">
                            <span className="agent-card-icon">
                              {agent.agent_type === 'local' ? '💻' :
                               ...
                               agent.agent_type === 'cloud' ? '☁️' : '🔌'}
                            </span>
                            <div>
                              <div className="agent-card-name">{agent.name}</div>
                              <div className="agent-card-subtitle">
                                {agent.agent_type.charAt(0).toUpperCase() + agent.agent_type.slice(1)} Agent
                                {agent.gpu_id && gpus.find(g => g.id === agent.gpu_id) && (
                                  <span className="agent-card-gpu">
                                    • GPU: {gpus.find(g => g.id === agent.gpu_id)?.name || agent.gpu_id}
                                  </span>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="agent-card-status">
                            {agent.is_active ? (
                              <span className={`agent-status-badge ${agent.is_available ? 'active' : 'inactive'}`}>
                                <span className="agent-status-dot"></span>
                                {agent.is_available ? 'Available' : 'Unavailable'}
                              </span>
                            ) : (
                              <span className="agent-status-badge disabled">
                                <span className="agent-status-dot"></span>
                                Disabled
                              </span>
                            )}
                          ...
                          <div className="agent-card-details">
                            {agent.model && (
                              <div className="agent-detail-item">
                                ...
                                <span className="agent-detail-value">{agent.model}</span>
                              </div>
                            )}
                            {agent.agent_type === 'remote' && agent.host && (
                              <div className="agent-detail-item">
                                <span className="agent-detail-label">Host:</span>
                                <span className="agent-detail-value">{agent.host}:{agent.port}</span>
                              </div>
                            )}
                            {agent.agent_type === 'cloud' && agent.endpoint && (
                              <div className="agent-detail-item">
                                <span className="agent-detail-label">Endpoint:</span>
                                <span className="agent-detail-value">{agent.endpoint}</span>
                              </div>
                            )}
                            {(agent.max_tokens || agent.temperature !== undefined) && (
                              <div className="agent-detail-item">
                                <span className="agent-detail-label">Config:</span>
                                <span className="agent-detail-value">
                                  {agent.max_tokens && `${agent.max_tokens} tokens`}
                                  {agent.max_tokens && agent.temperature !== undefined && ' • '}
                            ...
                            {agent.last_used && (
                              <div className="agent-detail-item">
                                <span className="agent-detail-label">Last used:</span>
                                <span className="agent-detail-value">
                        ...
                        <div className="agent-card-footer">
                          <div className="agent-card-actions">
                            <button
                              className="agent-action-btn agent-action-test"
                              onClick={async () => {
                                try {
                                  const { codeEditorApi } = await import('../../../services/api')
                                  await codeEditorApi.testAgent(agent.id)
                                  // Test successful - could show a toast notification here
                                  loadAgents() // Refresh to update status
                                } catch (err: any) {
                            ...
                            <button
                              className="agent-action-btn agent-action-edit"
                              onClick={() => {
                              ...
                              title="Edit agent"
                            >
                              <span className="agent-action-icon">✏️</span>
                              Edit
                            </button>
                            <button
            ...
            {settingsTab === 'appearance' && (
              <div className="settings-section">
                ...
                <div className="settings-subsection">
                  ...
                  {/* Platform Themes */}
                  <div className="form-group">
                    <label style={{ fontSize: '13px', fontWeight: 600, marginBottom: '8px', color: 'var(--theme-fg-secondary, #858585)' }}>Platform Themes</label>
                    <div className="theme-selector">
                      {AVAILABLE_THEMES.filter(t => t.id.startsWith('macos-') || t.id.startsWith('windows-') || t.id.startsWith('linux-')).map((themeDef) => (
                        <div
                          key={themeDef.id}
                  ...
                  <div className="form-group" style={{ marginTop: '24px' }}>
                    <label style={{ fontSize: '13px', fontWeight: 600, marginBottom: '8px', color: 'var(--theme-fg-secondary, #858585)' }}>Other Themes</label>
                    <div className="theme-selector">
                      {AVAILABLE_THEMES.filter(t => !t.id.startsWith('macos-') && !t.id.startsWith('windows-') && !t.id.startsWith('linux-')).map((themeDef) => (
                        <div
                          key={themeDef.id}
