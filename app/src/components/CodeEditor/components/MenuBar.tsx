...
interface MenuBarProps {
  ...
  setShowGitDialog: (show: boolean) => void
}
  ...
  const menuItems = [
    {
      ...
      items: [
        ...
        { divider: true },
        { label: 'Git', submenu: [
          { label: 'Clone Repository...', action: () => { setShowGitDialog(true); setTimeout(() => { const event = new CustomEvent('git-tab-change', { detail: 'clone' }); window.dispatchEvent(event); }, 100); } },
          { label: 'Push Code', action: () => { setShowGitDialog(true); setTimeout(() => { const event = new CustomEvent('git-tab-change', { detail: 'push' }); window.dispatchEvent(event); }, 100); } },
          { label: 'Pull Changes', action: () => { setShowGitDialog(true); setTimeout(() => { const event = new CustomEvent('git-tab-change', { detail: 'pull' }); window.dispatchEvent(event); }, 100); } },
          { divider: true },
          { label: 'Status', action: () => { setShowGitDialog(true); setTimeout(() => { const event = new CustomEvent('git-tab-change', { detail: 'status' }); window.dispatchEvent(event); }, 100); } },
          { label: 'Commit...', action: () => { setShowGitDialog(true); setTimeout(() => { const event = new CustomEvent('git-tab-change', { detail: 'commit' }); window.dispatchEvent(event); }, 100); } },
          { label: 'Initialize Repository', action: () => { setShowGitDialog(true); setTimeout(() => { const event = new CustomEvent('git-tab-change', { detail: 'init' }); window.dispatchEvent(event); }, 100); } },
        ]},
        { divider: true },
    ...
    },
    {
      name: 'Project',
      items: [
        { label: 'Project Settings...', shortcut: 'Ctrl+Shift+P', action: () => setShowProjectSettings(true) },
        { divider: true },
        { label: 'Switch Project...', shortcut: 'Ctrl+Shift+O', action: () => setShowProjectSwitcher(true) },
        { label: 'New Project...', shortcut: 'Ctrl+Shift+N', action: () => setShowNewProjectDialog(true) }
      ]
    },
    {
    ...
    },
    {
      name: 'Payment',
      items: [
        { label: 'Payment & Subscriptions...', shortcut: 'Ctrl+Shift+$', action: () => setShowPaymentDialog(true) }
      ]
    },
    {
      name: 'Help',
