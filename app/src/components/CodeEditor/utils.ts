...
// Compute line-by-line diff between old and new content using an improved algorithm
export const computeDiff = (oldContent: string, newContent: string): DiffLine[] => {
  const oldLines = oldContent.split('\n')
  const newLines = newContent.split('\n')
  const diff: DiffLine[] = []
  
  // Use a more sophisticated diff algorithm (simplified LCS-based approach)
  // This handles insertions, deletions, and modifications more accurately
  ...
  // Reconstruct the diff from the DP table
  let i = m
  let j = n
  const result: Array<{ type: 'added' | 'removed' | 'unchanged', oldIdx: number, newIdx: number }> = []
  
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && oldLines[i - 1] === newLines[j - 1]) {
      // Lines match
      result.unshift({ type: 'unchanged', oldIdx: i - 1, newIdx: j - 1 })
      i--
      j--
    } else if (j > 0 && (i === 0 || dp[i][j - 1] >= dp[i - 1][j])) {
      // Added line
      result.unshift({ type: 'added', oldIdx: -1, newIdx: j - 1 })
      j--
    } else if (i > 0) {
      // Removed line
      result.unshift({ type: 'removed', oldIdx: i - 1, newIdx: -1 })
      i--
    }
  }
...
export const parseMessageContent = (content: string): MessagePart[] => {
  ...
  // More strict regex: requires ``` at start, optional language, whitespace/newline, content, and ``` at end
  // Uses non-greedy matching and requires at least some whitespace after opening ```
  ...
  while ((match = codeBlockRegex.exec(content)) !== null) {
    ...
    // Validate that this is actually a code block
    ...
    // Check if content looks like plain text explanation rather than code
    const lines = codeContent.split('\n')
    const isSingleLine = lines.length === 1
    const wordCount = codeContent.split(/\s+/).filter(w => w.length > 0).length
    
    const looksLikePlainText = 
      isSingleLine && // Single line
      codeContent.length < 80 && // Short content
      wordCount > 2 && wordCount < 15 && // Has multiple words (likely prose)
      !/[{}();=<>[\]]/.test(codeContent) && // No code-like characters
      !/^(import|from|def |class |function |const |let |var |export |return |if |for |while |#|\/\/|\/\*|<!DOCTYPE|<\?xml)/m.test(codeContent) && // No code keywords
      /^[A-Z]/.test(codeContent.trim()) && // Starts with capital (likely sentence)
      codeContent.includes(' ') && // Has spaces (likely prose)
      !codeContent.match(/[a-zA-Z_$][a-zA-Z0-9_$]*\s*[:=]/) && // No variable assignments
      !codeContent.match(/[a-zA-Z_$][a-zA-Z0-9_$]*\s*\(/) && // No function calls
      !codeContent.match(/<[a-zA-Z]/) && // No HTML/XML tags
      codeContent.match(/[.!?]\s*$/) // Ends with sentence punctuation
    
    // Check if it's clearly code
    const hasCodePatterns = 
      /[{}();=<>[\]]/.test(codeContent) || // Has code-like characters
      /^(import|from|def |class |function |const |let |var |export |return |if |for |while |#|\/\/|\/\*|<!DOCTYPE|<\?xml)/m.test(codeContent) || // Code keywords or XML/HTML
      /^\s*[a-zA-Z_$][a-zA-Z0-9_$]*\s*[:=]/.test(codeContent) || // Variable assignment pattern
      /^[\s]*(<[a-zA-Z]|<\/[a-zA-Z]|<\?|<!)/m.test(codeContent) || // HTML/XML tags
      /^[\s]*(\{|\}|\[|\]|\(|\))/m.test(codeContent) || // Starts with brackets
      codeContent.match(/[a-zA-Z_$][a-zA-Z0-9_$]*\s*\([^)]*\)\s*[;{]/) || // Function calls
      codeContent.match(/\.[a-zA-Z_$][a-zA-Z0-9_$]*\s*\(/) || // Method calls
      codeContent.match(/^\s*\d+\s*[+\-*\/=]|\+\+|--/) || // Math operations
      codeContent.match(/^\s*\/\/|\/\*|\*\/|#\s/) // Comments
    
    const isLikelyCode = 
      codeContent.length > 3 && 
      !looksLikePlainText && (
        language.length > 0 || // Has language identifier
        (lines.length > 1 && codeContent.length > 10) || // Multi-line and substantial
        hasCodePatterns // Has code-like patterns
      )
    
    if (isLikelyCode) {
      parts.push({ type: 'code', content: codeContent, language })
    } else {
      // Treat as text if it doesn't look like code - preserve the backticks in the text
      parts.push({ type: 'text', content: `\`\`\`${language ? language + ' ' : ''}${codeContent}\`\`\`` })
    }
...
const parseTextContent = (text: string): MessagePart[] => {
  ...
  while (i < lines.length) {
    ...
    // Check for headers (# ## ###) - improved detection
    // Also handle headers with optional spacing and alternative formats
    let headerMatch = trimmedLine.match(/^(#{1,6})\s+(.+)$/)
    let isUnderlinedHeader = false
    let headerContent = ''
    let headerLevel = 3
    
    if (!headerMatch && trimmedLine.length > 0 && trimmedLine.length < 80 && i + 1 < lines.length) {
      // Check for underlined headers (Setext style)
      const nextLine = lines[i + 1].trim()
      if (nextLine.match(/^[=\-]{3,}$/)) {
        isUnderlinedHeader = true
      ...
      headerLevel = Math.min(headerMatch[1].length, 6)
      headerContent = headerMatch[2].trim()
    }
    
    if (headerMatch || isUnderlinedHeader) {
      if (currentText.trim()) {
        parts.push({ type: 'text', content: currentText.trim() })
        currentText = ''
      }
    ...
    // Check for lists - improved detection with better spacing handling
    ...
    if (bulletListMatch || numberedListMatch) {
      ...
      // Collect list items with better handling of nested content
      ...
      while (i < lines.length) {
        ...
        const bulletMatch = trimmedCurrent.match(/^[\*\-\•]\s+(.+)$/)
        const numberedMatch = trimmedCurrent.match(/^\d+[\.\)]\s+(.+)$/)
        const indentMatch = currentLine.match(/^(\s*)/)
        const currentIndent = indentMatch ? indentMatch[1].length : 0

        // Check if this is a continuation of previous list item (indented)
        if (trimmedCurrent && !bulletMatch && !numberedMatch && currentIndent > indentLevel && listItems.length > 0) {
          // Append to last item as continuation
          listItems[listItems.length - 1] += ' ' + trimmedCurrent
          i++
          continue
        }
    ...
    // Regular text line - check if it looks like code but isn't in a code block
    // If a line looks like code but is in regular text, it's probably just text mentioning code
    const looksLikeCodeLine = trimmedLine.match(/^[a-zA-Z_$][a-zA-Z0-9_$]*\s*[:=].*[;)]$/) ||
                              trimmedLine.match(/^(if|for|while|def|class|function|const|let|var)\s+/) ||
                              (trimmedLine.includes('()') && trimmedLine.includes('='))
    
    // If it looks like code but is standalone text, treat as text (not code)
    // This prevents single lines of code-like text from being misclassified
    if (looksLikeCodeLine && trimmedLine.length < 100) {
      // It's probably just text mentioning code, not actual code
      currentText += line + '\n'
    } else {
      currentText += line + '\n'
    }
...
const cleanCodeContent = (content: string): string => {
  ...
  // Handle escaped JSON strings - unescape \n, \t, etc.
  ...
  if (cleaned.startsWith('{') && cleaned.includes('"content"') && cleaned.includes('"operations"')) {
    try {
      // Try to extract content from JSON structure - handle both escaped and unescaped strings
      ...
      if (jsonMatch && (jsonMatch[1] || jsonMatch[2] || jsonMatch[3])) {
        let extracted = (jsonMatch[1] || jsonMatch[2] || jsonMatch[3] || '')
  ...
  // Replace __CODEPLACEHOLDER0__ with main_function (or detect from context)
  const functionNameMatch = cleaned.match(/\bdef\s+(\w+)\s*\(/g)
  if (functionNameMatch && functionNameMatch.length > 0) {
    // Extract the first function name found
    const firstFunction = functionNameMatch[0].match(/\bdef\s+(\w+)\s*\(/)?.[1]
    if (firstFunction) {
      // Replace placeholders with the actual function name
      cleaned = cleaned.replace(/__CODEPLACEHOLDER0__/g, firstFunction)
      cleaned = cleaned.replace(/__CODEPLACEHOLDER1__/g, firstFunction)
    }
  }
  
  // Fix function name inconsistencies (mainfunction vs main_function)
  // If we see both variations, standardize to snake_case
  const mainFunctionVariants = cleaned.match(/\b(mainfunction|main_function)\b/g)
  if (mainFunctionVariants) {
    // Check which variant appears more
    const mainfunctionCount = (cleaned.match(/\bmainfunction\b/g) || []).length
    const main_functionCount = (cleaned.match(/\bmain_function\b/g) || []).length
    
    // Standardize to the more common variant, or default to main_function (Python convention)
    if (mainfunctionCount > main_functionCount) {
      cleaned = cleaned.replace(/\bmain_function\b/g, 'mainfunction')
    } else {
      cleaned = cleaned.replace(/\bmainfunction\b/g, 'main_function')
    }
  }
  ...
  // Fix common indentation issues - ensure consistent spacing
  // Don't change indentation if it looks intentional, but fix obvious issues
  const indentLines = cleaned.split('\n')
  if (indentLines.length > 0) {
    // Detect indentation style from first non-empty line
    const firstNonEmpty = indentLines.find(line => line.trim().length > 0)
    if (firstNonEmpty) {
      const indentMatch = firstNonEmpty.match(/^(\s+)/)
      if (indentMatch) {
        const indentChar = indentMatch[1][0] === '\t' ? '\t' : ' '
        const indentSize = indentMatch[1].length
        
        // Normalize indentation (but be conservative - only fix obvious issues)
        const normalizedLines = indentLines.map(line => {
          if (!line.trim()) return line // Preserve blank lines
          
          // If line has mixed tabs/spaces, convert to detected style
          if (line.includes('\t') && indentChar === ' ') {
            // Convert tabs to spaces
            line = line.replace(/\t/g, ' '.repeat(indentSize))
          } else if (!line.includes('\t') && indentChar === '\t') {
            // Convert spaces to tabs (be more careful here)
            const leadingSpaces = line.match(/^(\s+)/)?.[1] || ''
            if (leadingSpaces.length > 0 && leadingSpaces.length % indentSize === 0) {
              line = line.replace(/^(\s+)/, (match) => '\t'.repeat(match.length / indentSize))
            }
          }
...
export const validateFileOperation = (operation: FileOperation, findFile: (path: string) => FileNode | null): {
  ...
  if (operation.type === 'delete') {
    ...
    if (operation.language && 
        !['edit_file', 'create_file', 'delete_file'].includes(operation.language.toLowerCase()) &&
        operation.language !== detectedLang && 
    ...
    // Fix language if it's a special marker - use detected language from path
    if (operation.language && ['edit_file', 'create_file', 'delete_file'].includes(operation.language.toLowerCase())) {
      operation.language = detectedLang
    }
...
export const parseAIResponse = (content: string, findFile: (path: string) => FileNode | null): ParsedAIResponse => {
        ...
        const opsWithLang = opsData.operations.map((op: any) => {
          ...
          if (cleanedContent && typeof cleanedContent === 'string') {
            ...
            if (op.path.endsWith('.py') || detectLanguageFromPath(op.path) === 'python') {
              ...
              // Fix __name__ == '__main__' pattern - catch more variations
              cleanedContent = cleanedContent.replace(/\bif\s+name\s+==\s+['"]main['"]:/gi, "if __name__ == '__main__':")
              cleanedContent = cleanedContent.replace(/\bif\s+name\s*==\s*['"]main['"]:/gi, "if __name__ == '__main__':")
              cleanedContent = cleanedContent.replace(/\bif\s+__name__\s*==\s*['"]main['"]:/g, "if __name__ == '__main__':")
              ...
              const mainFunctionMatch = cleanedContent.match(/\bdef\s+(mainfunction|main_function)\s*\(/i)
              if (mainFunctionMatch) {
                const functionName = mainFunctionMatch[1]
                // Standardize all references to use the same name
                if (functionName === 'mainfunction') {
                  cleanedContent = cleanedContent.replace(/\bmain_function\b/g, 'mainfunction')
                } else {
                  cleanedContent = cleanedContent.replace(/\bmainfunction\b/g, 'main_function')
                }
              }
            }
          }
  ...
  while ((match = editFileRegex.exec(content)) !== null) {
    ...
    if (detectedLang === 'python' || filePath.endsWith('.py')) {
      ...
      // Fix __name__ == '__main__' pattern - catch more variations
      fileContent = fileContent.replace(/\bif\s+name\s+==\s+['"]main['"]:/gi, "if __name__ == '__main__':")
      fileContent = fileContent.replace(/\bif\s+name\s*==\s*['"]main['"]:/gi, "if __name__ == '__main__':")
      fileContent = fileContent.replace(/\bif\s+__name__\s*==\s*['"]main['"]:/g, "if __name__ == '__main__':")
      fileContent = fileContent.replace(/\bfrom\s+importlib\s+import\s+importmodule\b/g, 'from importlib import import_module')
      
      // Fix function name inconsistencies
      const mainFunctionMatch = fileContent.match(/\bdef\s+(mainfunction|main_function)\s*\(/i)
      if (mainFunctionMatch) {
        const functionName = mainFunctionMatch[1]
        // Standardize all references to use the same name
        if (functionName === 'mainfunction') {
          fileContent = fileContent.replace(/\bmain_function\b/g, 'mainfunction')
        } else {
          fileContent = fileContent.replace(/\bmainfunction\b/g, 'main_function')
        }
      }
    }
  ...
  // Skip edit_file, create_file, delete_file patterns as they're handled above
  const codeBlockWithPathRegex = /```(\w+)?:?([\/\w\.\-]+\.\w+)\n([\s\S]*?)```/g
  while ((match = codeBlockWithPathRegex.exec(content)) !== null) {
    let codeLanguage = match[1] || ''
    const potentialPath = match[2]
    let codeContent = match[3].trim()
    
    // Skip if this is an edit_file, create_file, or delete_file block (already handled)
    if (codeLanguage && ['edit_file', 'create_file', 'delete_file'].includes(codeLanguage.toLowerCase())) {
      continue
    }
    ...
    // Detect language from file path first, use codeLanguage only if path doesn't provide one
    ...
    // Clean the code content first
    let cleanedCodeContent = cleanCodeContent(codeContent)
    ...
    if (potentialPath.startsWith('/') || potentialPath.includes('/') || potentialPath.match(/\.\w+$/)) {
      ...
      // Detect expected language from file path (this is the source of truth)
      const expectedLang = detectLanguageFromPath(filePath)
      
      // Only validate if codeLanguage is provided and it's not a special marker
      // Use expectedLang from file path as the actual language
      if (codeLanguage && codeLanguage !== expectedLang && 
          !['edit_file', 'create_file', 'delete_file'].includes(codeLanguage.toLowerCase()) &&
          expectedLang !== 'plaintext') {
        // Only warn if there's a real mismatch, not for special markers
      ...
      if (existingFile) {
        ...
        operations.push({
          ...
          oldContent: existingFile.content || '',
          description: `Edit ${filePath}`,
          language: expectedLang // Always use language from file path
        })
      } else {
        // It's a create
        operations.push({
